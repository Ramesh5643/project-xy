...
type GetStyleInfo = (resolved: { element: HTMLElement | null }) => {
  className: string;
...
function initDesignMode(getStyleInfo: GetStyleInfo) {
  ...
  const reselect = () => {
    try {
      const resolved = { element: document.activeElement as HTMLElement | null };
      getStyleInfo(resolved);
    } catch {
      // no-op when design mode isn't active or the selected element is unavailable
    }
  };
